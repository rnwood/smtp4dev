...
namespace Rnwood.Smtp4dev.Data
{
    public class MessagesRepository : IMessagesRepository
    {
        ...
        {
            var query = dbContext.Messages.Where(m => m.MailboxFolder.Mailbox.Name == mailboxName && m.MailboxFolder.Name == folderName);
            return unTracked ? query.AsNoTracking() : query;
        }
        ...
        {
            return dbContext.Messages.Where(m => m.MailboxFolder.Mailbox.Name == mailboxName && m.MailboxFolder.Name == folderName)
                .Select(m => new MessageSummaryProjection()
                {
                    Id = m.Id,
                    From = m.From,
