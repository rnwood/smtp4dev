    ...
    public static class CommandLineParser
    {
        public static MapOptions<CommandLineOptions> TryParseCommandLine(IEnumerable<string> args, bool isDesktopApp)
        {
            ...
            OptionSet options = new OptionSet
            {
                ...
                { "imapport=", "Specifies the port the IMAP server will listen on - allows standard email clients to view/retrieve messages. Set to null in config file to disable the IMAP server.", data => map.Add(data, x => x.ServerOptions.ImapPort) },
                { "pop3port=", "Specifies the port the POP3 server will listen on - allows standard email clients to retrieve messages. Set to null in config file to disable the POP3 server.", data => map.Add(data, x => x.ServerOptions.Pop3Port) },
