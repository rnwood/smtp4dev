...
export default class MessagesController {
    ...
    // post: api/Messages/${encodeURIComponent(id)}/replyWithAttachments
    public replyWithAttachments_url(id: string, from: string, to: string, cc: string, bcc: string, deliverToAll: boolean, subject: string): string {
        return `${this.apiBaseUrl}/${encodeURIComponent(id)}/replyWithAttachments?from=${encodeURIComponent(from)}&to=${encodeURIComponent(to)}&cc=${encodeURIComponent(cc)}&bcc=${encodeURIComponent(bcc)}&deliverToAll=${encodeURIComponent(deliverToAll)}&subject=${encodeURIComponent(subject)}`;
    }

    public async replyWithAttachments(id: string, from: string, to: string, cc: string, bcc: string, deliverToAll: boolean, subject: string, bodyHtml: string, attachments: File[]): Promise<void> {
        ...
        return (await axios.post(this.replyWithAttachments_url(id, from, to, cc, bcc, deliverToAll, subject), formData, { 
    ...
    // post: api/Messages/sendWithAttachments
    public sendWithAttachments_url(from: string, to: string, cc: string, bcc: string, deliverToAll: boolean, subject: string): string {
        return `${this.apiBaseUrl}/sendWithAttachments?from=${encodeURIComponent(from)}&to=${encodeURIComponent(to)}&cc=${encodeURIComponent(cc)}&bcc=${encodeURIComponent(bcc)}&deliverToAll=${encodeURIComponent(deliverToAll)}&subject=${encodeURIComponent(subject)}`;
    }

    public async sendWithAttachments(from: string, to: string, cc: string, bcc: string, deliverToAll: boolean, subject: string, bodyHtml: string, attachments: File[]): Promise<void> {
        const formData = new FormData();
        formData.append('bodyHtml', bodyHtml);
        ...
        return (await axios.post(this.sendWithAttachments_url(from, to, cc, bcc, deliverToAll, subject), formData, { 
            headers: { "Content-Type": "multipart/form-data" } 
        })).data as void;
    }
