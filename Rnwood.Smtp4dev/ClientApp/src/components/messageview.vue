    ...
    <div v-if="messageSummary" class="hfillpanel fill">
      <div class="toolbar">
        <el-button icon="el-icon-download"
                   v-on:click="download">Download
        </el-button>

        <el-button v-on:click="relay" icon="el-icon-d-arrow-right" :disabled="!messageSummary || !isRelayAvailable"
                   :loading="isRelayInProgress" title="Relay">Relay
        </el-button>
      </div>
      ...
      <el-tabs value="view" style="height: 100%; width:100%" class="fill" type="border-card">
        ...
        <el-tab-pane label="Raw" name="raw" class="fill vfillpanel">
          <messagesource class="fill" :message="message" type="raw"></messagesource>
        </el-tab-pane>
      </el-tabs>
    </div>
