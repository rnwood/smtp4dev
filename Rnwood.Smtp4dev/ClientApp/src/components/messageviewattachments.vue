<template>
    <div class="vfillpanel pad" v-show="attachments">
        <div v-for="attachment in attachments" :key="attachment.id"><el-button size="small" icon="el-icon-document" type="primary" round v-on:click="openAttachment(attachment)">{{attachment.fileName}}</el-button></div>
    </div>
</template>
<script lang="ts">
